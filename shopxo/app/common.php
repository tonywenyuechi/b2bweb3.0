...
function AdminIsPower($controller = null, $action = null, $unwanted_power = [])
{
    ...
    // 登录相关页面和错误提示页面，未登录时直接通过
    $login_pages = [
        'admin' => ['logininfo', 'login', 'logout', 'adminverifyentry', 'loginverifysend', 'saveinfo', 'save'],
        'error' => ['tips']
    ];
    if(array_key_exists($controller, $login_pages) && in_array($action, $login_pages[$controller]))
    {
        return true;
    }
...
function PathToParams($key = null, $default = null, $path = '')
{
    ...
    if(!empty($path) && $key !== null && !array_key_exists($key, $data))
    {
        if(substr($path, 0, 1) == '/')
        {
